...
export const BlogCard = ({tag, link, title, excerpt}) => {
    return (
        <div className="flex flex-col md:flex-row rounded-lg shadow-md mb-5">
            <GatsbyImage 
                className="w-auto md:w-48 rounded-lg"
            />
            <div className="flex-col p-5 content-center">
              <span className="text-sm font-bold text-green-500">#{tag}</span>
              ...
              <excerpt className="text-md font-regular text-gray-500">{excerpt}</excerpt>
            </div>
        </div>
    )
}
